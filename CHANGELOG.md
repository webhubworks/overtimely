...
### Changed
- **Breaking:** The user config file now mirrors the structure of Laravel's config repository, so `account_id` is stored as `{"timely": {"account_id": ...}}` instead of at the top level. Settings written by earlier versions are not migrated: note your current settings, upgrade, then run `app:setup` again. Stale top-level keys are removed the next time the file is written.
- **Breaking:** The `TABLE_STYLE` environment variable is now `DISPLAY_TABLE_STYLE`, matching the `display.table_style` entry it overrides. Every other setting already followed this pattern.
